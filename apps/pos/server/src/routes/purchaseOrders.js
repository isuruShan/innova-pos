...
router.post('/:id/send', protect, tenantScope, resolveSelectedStore, async (req, res) => {
  try {
    ...
    let pdfBuffer;
    try {
      pdfBuffer = await generatePurchaseOrderPDF(order, order.supplierId, store || { name: 'Merchant' });
    } catch (pdfErr) {
    ...
    if (order.supplierId && order.supplierId.email) {
      const emailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
          ...
          <p>Please find attached the Purchase Order <strong>${order.orderNumber}</strong> generated by <strong>${store?.name || 'Merchant'}</strong>.</p>
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            ...
            <tr>
              ...
              <td style="padding: 8px 0; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: bold; color: #ea580c;">$${order.totalAmount.toFixed(2)}</td>
            </tr>
          </table>
          ...
          <p>
            <strong>${store?.name || 'Merchant'}</strong><br>
            ${store?.phone ? `Phone: ${store.phone}<br>` : ''}
            ${store?.address?.street1 || ''} ${store?.address?.city || ''}
          </p>
          
          <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 25px 0;" />
          <p style="color: #64748b; font-size: 12px; text-align: center;">This is an automated purchase order request generated on behalf of ${store?.name || 'Merchant'}.</p>
        </div>
      `;
