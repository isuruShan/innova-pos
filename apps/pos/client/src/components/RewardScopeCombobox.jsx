...
export default function RewardScopeCombobox({
  ...
  if (!isStoreReady) {
    return <p className="text-xs text-slate-500">Choose a store in the header to scope items.</p>;
  }
  if (!menuItems.length) {
    return <p className="text-xs text-slate-500">No menu items for this store yet.</p>;
  }
      ...
      <div className="relative">
        <div className="flex items-center gap-2 bg-[var(--pos-surface-inset)] border border-slate-700 rounded-xl px-3 py-2 focus-within:border-amber-500 transition">
          ...
          {search ? (
            <button
              ...
              className="p-0.5 rounded text-slate-500 hover:text-[var(--pos-text-primary)]"
        ...
        {showDrop && (
          <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-[var(--pos-panel)] border border-slate-700 rounded-xl shadow-xl overflow-hidden max-h-56 overflow-y-auto">
            {matchCats.length > 0 && (
              <>
                <div className="px-3 py-1.5 text-xs font-semibold text-slate-500 bg-slate-800/60 sticky top-0">
                  Categories
                </div>
                {matchCats.map((c) => (
                  <button
                  ...
                  >
                    ...
                    <span
                      className={
                        categoryNames.includes(c) ? 'text-amber-400 font-medium' : 'text-slate-300'
                      }
                    >
                      {c}
                    </span>
        ...
        {showNoResults ? (
          <p className="text-xs text-slate-500 mt-1.5 px-0.5">No categories or products match.</p>
        ) : null}
      </div>
      ...
      {totalSelected > 0 ? (
        <div className="flex flex-wrap gap-1.5">
          {categoryNames.map((c) => (
            <span
              key={`cat-${c}`}
              className="inline-flex items-center gap-1 bg-amber-500/20 border border-amber-500/30 text-amber-400 text-xs px-2.5 py-1 rounded-full"
            >
              📁 {c}
              <button
                type="button"
                onClick={() => removeCategory(c)}
                className="ml-0.5 hover:text-[var(--pos-text-primary)] leading-none"
                aria-label={`Remove ${c}`}
              >
                ×
              </button>
          ...
          {itemIds.map((id, i) => (
            <span
              key={`item-${String(id)}`}
              className="inline-flex items-center gap-1 bg-slate-700 border border-slate-600 text-slate-200 text-xs px-2.5 py-1 rounded-full"
            >
              {itemNames[i] ?? 'Item'}
              <button
                type="button"
        ...
        <p className="text-xs text-slate-500">
          Nothing selected — discount applies to the whole order (when scoped types allow).
        </p>
      )}
    </div>
  );
