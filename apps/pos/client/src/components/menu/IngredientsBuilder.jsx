...
export default function IngredientsBuilder({ menuItemId, storeId }) {
  ...
  return (
    <div className="space-y-3">
              ...
              <div key={link._id} className="flex items-center gap-2 px-3 py-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-slate-200 truncate">{inv?.itemName || 'Unknown'}</span>
                    {isLowStock && (
      ...
      {ingredientLinks.length === 0 && (
        <div className="text-center py-6 text-slate-600 bg-[var(--pos-surface-inset)] rounded-xl">
          <Package size={24} className="mx-auto mb-2 opacity-50" />
          <p className="text-sm">No ingredients linked</p>
      ...
      {availableInventory.length > 0 ? (
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 bg-amber-500 hover:bg-amber-400 disabled:opacity-60 text-white font-medium px-3 py-2 rounded-lg transition text-sm"
          >
