...
export default function VariantSelectorModal({ item, onClose, onConfirm, orderType, partners, getItemPrice }) {
  ...
  const { data: modifierGroups = [] } = useQuery({
    ...
    // Only fetch when item has modifier groups linked and addon is active
    enabled: isStoreReady && modifierGroupsActive && !!item?.modifierGroups?.length,
  });

  // ── Derived values (all useMemo calls must be unconditional) ───────────────
  ...
  const finalSelectedModifiers = useMemo(() => {
    return selectedMods.map((m) => {
      ...
      // Apply variant-specific price override when available
      const link = item?.modifierGroups?.find((g) => String(g.modifierGroupId) === String(m.modifierGroupId));
      const override = link?.overrides?.find(
        (o) =>
          String(o.modifierId) === String(m.modifierId) &&
          String(o.variantId || '') === String(selectedVariant?._id || ''),
      );
      const price = override ? override.price : defaultPrice;
      return { ...m, price };
    });
  }, [selectedMods, selectedVariant, linkedGroups, item?.modifierGroups]);
  ...
  const modifiersTotalPrice = useMemo(
    () => finalSelectedModifiers.reduce((sum, m) => sum + m.price * (m.qty || 1), 0),
    ...
    setSelectedMods((prev) => {
      ...
      if (group.maxSelections && groupSelectedCount >= group.maxSelections) {
        if (group.maxSelections === 1) {
          // Single-select: swap the current choice
          return [
        ...
        return prev; // Multi-select limit reached
      }
                  ...
                  {group.modifiers?.map((opt) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {opt.available === false && (
                          <span className="text-[8px] uppercase tracking-wider bg-red-950/20 text-red-500 border border-red-900/20 px-1 py-0.5 rounded shrink-0">
                            Sold Out
                          </span>
                        )}
                      </button>
        ...
        <div className="bg-[var(--pos-surface-inset)] rounded-xl p-3 border border-slate-800 flex items-center gap-3 shrink-0 mt-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            {finalSelectedModifiers.length > 0 ? (
              ...
              <p className="text-xs text-slate-500 truncate mt-0.5">
                {selectedVariant?.description || item.description || 'No customization'}
              </p>
            )}
          </div>
