...
export default function InventoryManagement() {
  ...
  const lowCount = activeTab === 'stock' && selectedCategoryId !== null
    ? (inventoryData.summary?.lowStockCount || 0)
    : (Array.isArray(inventoryData)
        ? inventoryData.filter(i => getStockStatus(i.quantity, i.minThreshold).variant !== 'ok').length
        : (inventoryData.summary?.lowStockCount || 0));
    ...
    <div className="min-h-screen bg-[var(--pos-page-bg)]">
                  ...
                  <ResponsiveTable
                    ...
                    columns={[
                      ...
                      },
                      {
                        ...
                        className: 'text-right',
                        headerClassName: 'text-right',
                        sortField: FORMULA_FIELDS[selectedFormula],
                        render: (item) => {
                          const cost = item[FORMULA_FIELDS[selectedFormula]] || 0;
                          return <span className="text-[var(--pos-text-primary)] font-medium">{formatCurrency(cost)}</span>;
                        },
                      },
                      {
                        ...
                        return (
                          <div key={item._id} className="bg-[var(--pos-panel)] border border-slate-700/50 rounded-xl p-3.5 flex flex-col justify-between hover:border-slate-600 transition shadow-lg">
                            <div>
                              ...
                              <div className="grid grid-cols-3 gap-2 mt-3 bg-[var(--pos-surface-inset)] rounded-lg p-2.5 text-xs border border-slate-800/60">
                                <div>
                                ...
                                <div>
                                  <p className="text-[10px] text-slate-500">Unit Cost ({FORMULA_LABELS[selectedFormula]})</p>
                                  <p className="font-semibold text-slate-200">{formatCurrency(item[FORMULA_FIELDS[selectedFormula]] || 0)}</p>
                                </div>
                              </div>
                              <div className="mt-3">
      ...
      >
        <div className="space-y-6">
          ...
          >
            <div>
              <label className="block text-xs font-bold text-slate-400 mb-1">New Storage Area Name *</label>
              <input
                type="text"
                ...
                required
                className="w-full bg-[var(--pos-panel)] border border-slate-700 text-[var(--pos-text-primary)] rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-amber-500 placeholder-slate-500"
              />
            </div>
            {storageAreaError && (
              <p className="text-xs text-red-450 font-semibold">{storageAreaError}</p>
            )}
            <button
              type="submit"
              disabled={createStorageAreaMutation.isPending}
              className="w-full bg-amber-500 hover:bg-amber-400 text-white font-semibold py-2 rounded-xl transition text-xs disabled:opacity-60"
            >
          ...
          <div className="space-y-3">
            ...
            {storageAreasPending ? (
              <div className="text-xs text-slate-500 text-center py-4">Loading storage areas...</div>
              ...
              <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-1">
                {storageAreas.map(area => (
                  <div
                    key={area._id}
                    className="flex items-center justify-between bg-[var(--pos-panel)] border border-slate-700/50 hover:border-slate-600 rounded-xl p-3 gap-3"
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-[var(--pos-text-primary)] truncate">{area.name}</p>
                    </div>
                    <div className="flex gap-1 shrink-0">
                      <button
                        type="button"
