...
function ChargesTab() {
  ...
  if (isLoading || !local) return <div className="text-slate-500 py-10 text-center text-sm">Loading…</div>;
        ...
        {ORDER_TYPE_ROWS.map(({ key, label, icon }, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-1 bg-[#0f172a] border border-slate-700 rounded-lg px-2 py-1.5 focus-within:border-amber-500">
                <input
                  type="number" min="0" max="100" step="0.1"
                  value={cfg.taxRate}
                  disabled={!cfg.enabled}
                  onChange={e => set(key, 'taxRate', Math.max(0, Math.min(100, parseFloat(e.target.value) || 0)))}
                  className="w-full bg-transparent text-white text-sm text-center focus:outline-none disabled:cursor-not-allowed"
                />
                <Percent size={11} className="text-slate-600 flex-shrink-0" />
              </div>

              <div className="flex items-center gap-1.5">
                <div className="flex bg-[#0f172a] border border-slate-700 rounded-lg overflow-hidden flex-shrink-0">
                  <button
                    disabled={!cfg.enabled}
                    onClick={() => set(key, 'serviceFeeType', 'percentage')}
                    className={`px-2 py-1 text-xs font-medium transition ${!isFixed ? 'bg-amber-500 text-white' : 'text-slate-500 hover:text-white'} disabled:cursor-not-allowed`}
                ...
                <div className="flex-1 flex items-center gap-1 bg-[#0f172a] border border-slate-700 rounded-lg px-2 py-1.5 focus-within:border-amber-500 min-w-0">
...
function UserAvatar({ user }) {
  ...
  return (
    <div className={`w-10 h-10 rounded-full ${AVATAR_COLORS[user.role] || 'bg-slate-600'} flex items-center justify-center font-bold text-white text-xs flex-shrink-0`}>
        ...
        {[{ key: 'all', label: 'All' }, { key: 'cashier', label: 'Cashiers' }, { key: 'kitchen', label: 'Kitchen' }].map(f => (
          <button key={f.key} onClick={() => setFilterRole(f.key)}
            className={`px-3 py-1 rounded-full text-sm font-medium transition ${
              filterRole === f.key ? 'bg-amber-500 text-white' : 'text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700'
            }`}>{f.label}</button>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {filtered.map(user => {
            ...
            return (
              <div key={user._id} className="bg-[#1e293b] border border-slate-700/50 rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => openEdit(user)} className="p-1.5 rounded-lg text-slate-500 hover:text-white hover:bg-slate-700 transition"><Edit2 size={13} /></button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                return (
                  <button key={role} type="button" onClick={() => setForm(f => ({ ...f, role }))}
                    className={`flex items-center gap-2 px-3 py-3 rounded-xl border text-sm font-semibold transition ${
                      form.role === role ? `${cfg.bg} ${cfg.text} ${cfg.border}` : 'bg-[#0f172a] border-slate-700 text-slate-400 hover:border-slate-600'
                    }`}>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={closeSlide}
              className="flex-1 bg-slate-700 hover:bg-slate-600 text-white font-semibold py-2.5 rounded-xl transition text-sm">Cancel</button>
  ...
  return (
    <div className="min-h-screen bg-[#0f172a]">
      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-6">
        ...
        {/* Tab bar */}
        <div className="flex gap-1 bg-[#1e293b] border border-slate-700/50 rounded-xl p-1 mb-6 w-fit">
          {TABS.map(t => {
            const Icon = t.icon;
            return (
              <button
        ...
        {tab === 'charges' ? <ChargesTab /> : <UsersTab />}
      </div>
    </div>
  );
}
