...
router.post('/:id/points', protect, authorize('merchant_admin'), tenantScope, requireLoyalty, async (req, res) => {
  try {
...
router.get('/:id/points/history', protect, authorize('merchant_admin'), tenantScope, async (req, res) => {
  try {
