    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-4">
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-md max-w-sm w-full text-center space-y-4">
          <AlertTriangle size={40} className="mx-auto text-brand-orange animate-pulse" />
          <h2 className="text-lg font-bold text-gray-900">Select a Store</h2>
          <p className="text-sm text-gray-500">Please select a store to view and manage wastage.</p>
          <select
                  ...
                  <div className="grid grid-cols-12 gap-3 items-end">
                    ...
                    <div className="col-span-12 sm:col-span-5">
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-between bg-white border border-gray-200 text-gray-900 rounded-lg px-2.5 py-2.5 text-xs text-left hover:bg-gray-50 focus:outline-none focus:ring-1 focus:ring-amber-500 transition"
                      >
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
              ...
              {productSearch && (
                <button onClick={() => setProductSearch('')}>
                  <X size={13} className="text-gray-400 hover:text-gray-900" />
                </button>
              )}
            </div>

            {/* List of items */}
            <div className="max-h-[300px] overflow-y-auto divide-y divide-gray-100 border border-gray-150 rounded-xl bg-white">
              {(() => {
                ...
                if (row.itemType === 'menu') {
                  ...
                  return filtered.map((opt, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center justify-between text-xs transition ${
                          isSelected ? 'bg-amber-50 text-amber-900 font-semibold border-l-2 border-amber-500' : 'text-gray-700'
                        }`}
                      >
                        <div className="flex items-center gap-2">
                          <UtensilsCrossed size={14} className={isSelected ? 'text-amber-600' : 'text-gray-400'} />
                          <span className="truncate">{opt.name}</span>
                        </div>
                        {opt.category && (
                          <span className="bg-gray-100 text-gray-650 px-2 py-0.5 rounded text-[10px]">
                            {opt.category}
                          </span>
                        )}
                      </button>
                  ...
                  return filtered.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <Package size={14} className={isSelected ? 'text-amber-600' : 'text-gray-400'} />
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold px-4 py-2 rounded-xl text-sm"
              >
                Close
              </button>
            </div>
