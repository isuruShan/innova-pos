    ...
    <div className={className}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-150">
              {loading
                ...
                ? (
                    <tr>
                      <td colSpan={columns.length} className="py-16 text-center text-gray-400 text-sm bg-white">
                ...
                : rows.map((row) => (
                    <tr
                      ...
                      className="hover:bg-gray-50/50 transition bg-white"
                    >
                      {columns.map((col) => (
                        <td
                          key={col.key}
      ...
      <div className="sm:hidden">
        {loading ? (
          ...
          <div className="py-16 text-center text-gray-400 text-sm bg-white border border-gray-200 rounded-xl">
            {emptyState || 'No data found'}
          </div>
        ) : (
          <div className={`grid gap-3 ${mobileGridCols}`}>
            {rows.map((row) => {
              ...
              return (
                <div
                  key={key}
                  className="bg-white rounded-xl border border-gray-200 p-3.5 flex flex-col gap-2 shadow-sm"
                >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {rightCol.length > 0 && (
                        <div className="text-right shrink-0 flex flex-col items-end gap-0.5">
                          {rightCol.map((col) => (
                            <div key={col.key} className="text-sm font-semibold text-gray-900">
                              {col.render(row)}
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 border-t border-gray-100 pt-2">
                      {bodyFields.map((col) => (
                        <div key={col.key} className="min-w-0">
                          ...
                          <div className="text-xs text-gray-700">{col.render(row)}</div>
                        </div>
                      ))}
                    </div>
                  )}
