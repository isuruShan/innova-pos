/**
 * Browser-safe ESM entry (Vite / React clients).
 */
...
export function pickErrors(entries) {
  const errors = {};
...
export function validateBusinessName(value, { required = true } = {}) {
  const v = trim(value);
...
export function validatePassword(value, { required = true, min = LIMITS.passwordMin } = {}) {
...
export function validateMobile(value, countryIso = 'LK', { required = true } = {}) {
  const v = trim(value);
...
export function validateAddressLine(value, { required = true, label = 'Address' } = {}) {
  const v = trim(value);
  if (!v) return required ? fail(`${label} is required`) : ok();
...
export function validateRegion(value, { required = true, label = 'State / province' } = {}) {
  const v = trim(value);
  if (!v) return required ? fail(`${label} is required`) : ok();
...
export function validateRegistrationNumber(value, { required = false } = {}) {
  const v = trim(value);
...
export function validateMessage(value, { required = true, label = 'Message' } = {}) {
  const v = trim(value);
  if (!v) return required ? fail(`${label} is required`) : ok();
...
export function validateNotes(value, { required = false, max = LIMITS.notes, label = 'Notes' } = {}) {
  const v = trim(value);
  if (!v) return required ? fail(`${label} is required`) : ok();
...
export function validateTagline(value, { required = false } = {}) {
  const v = trim(value);
  if (!v) return ok();
...
export function validatePhoneDisplay(value, { required = false } = {}) {
  const v = trim(value);
...
export function validateStoreName(value, { required = true } = {}) {
  const v = trim(value);
...
export function validateStoreCode(value, { required = true } = {}) {
  const v = trim(value);
...
export function validateSignupPersonal({ firstName, lastName, email }) {
  return pickErrors([
...
export function validateSignupBusiness(form, { isRegistered = false } = {}) {
  const f = form || {};
...
export function fieldAttrs(type, options = {}) {
