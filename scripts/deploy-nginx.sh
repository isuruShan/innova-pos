...
for conf in pos admin-portal public-web qr-order central-kitchen; do
...
echo "  - kitchen.cafinity.io → Central Kitchen (port 5005)"

if $FIRST_DEPLOY; then
    echo ""
    echo "⚠  First deploy detected — add HTTPS with:"
    echo "   sudo certbot --nginx --expand -d www.cafinity.io -d cafinity.io -d pos.cafinity.io -d admin.cafinity.io -d shop.cafinity.io -d kitchen.cafinity.io"
fi
