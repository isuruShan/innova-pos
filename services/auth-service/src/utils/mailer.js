...
const sendEmail = async ({ to, subject, html, text }) => {
  ...
  const from = `InnovaPOS <${process.env.EMAIL_FROM || 'innovasolutionslk@gmail.com'}>`;
...
const sendWelcomeEmail = async ({ to, name, tempPassword, loginUrl }) => {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #1a1a2e;">Welcome to InnovaPOS, ${name}!</h2>
  ...
  `;
  await sendEmail({ to, subject: 'Your InnovaPOS Account is Ready', html });
};
...
const sendRejectionEmail = async ({ to, name, reason }) => {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Thank you for your interest in InnovaPOS.</p>
...
const sendPasswordResetEmail = async ({ to, name, resetUrl }) => {
  ...
  `;
  await sendEmail({ to, subject: 'Reset Your InnovaPOS Password', html });
};
